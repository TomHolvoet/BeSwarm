...
public class WiggleTrajectory extends BasicTrajectory implements FiniteTrajectory4d {
    private static final double WIGGLE_VELOCITY = 2d;
    ...
    WiggleTrajectory(Point4D centerPoint, int wiggles, double distance) {
        ...
        for (int i = 0; i < wiggles; i++) { //todo try hold + forTime
            ...
            //          new AbstractUncheckedStraightLineTrajectory4D(centerPoint, endRight,
            // WIGGLE_VELOCITY));
            //      builder.withTrajectory(
            //          new AbstractUncheckedStraightLineTrajectory4D(endRight, endLeft,
            // WIGGLE_VELOCITY));
            //      builder.withTrajectory(
            //          new AbstractUncheckedStraightLineTrajectory4D(endLeft, centerPoint,
            // WIGGLE_VELOCITY));

            builder.withTrajectory(
                    Trajectories.newHoldPositionTrajectory(endRight))
                    .forTime(distance / WIGGLE_VELOCITY);
            builder.withTrajectory(
                    Trajectories.newHoldPositionTrajectory(endLeft))
                    .forTime(2 * distance / WIGGLE_VELOCITY);
            builder.withTrajectory(
                    Trajectories.newHoldPositionTrajectory(centerPoint))
                    .forTime(distance / WIGGLE_VELOCITY);
        }
