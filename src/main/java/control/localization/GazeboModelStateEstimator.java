...
public final class GazeboModelStateEstimator implements StateEstimator {
    ...
    private GazeboModelStateEstimator(MessagesSubscriberService<ModelStates> modelStateSubscriber,
            String modelName) {
    ...
    /**
     * Creates a state estimator that use the ModelStates topics in Gazebo to get the current
     * state of the drone.
     *
     * @param modelStateSubscriber the rostopic subscriber to a topic publishing model state
     *     messages
     * @param modelName the name of the drone model. A model state topic in Gazebo contains the
     *     states of all models in the simulation environment, while the drone is one of those
     *     models.
    ...
    public Optional<DroneStateStamped> getCurrentState() {
        final Optional<ModelStates> modelStateOptional = modelStateSubscriber
        ...
        final InertialFrameVelocity inertialFrameVelocity = getInertialFrameVelocity(modelStates,
                index);
    ...
    private static InertialFrameVelocity getInertialFrameVelocity(ModelStates modelStates,
            int index) {
    ...
    private static Pose convertGazeboPoseToDronePose(geometry_msgs.Pose gazeboPose) {
        ...
        final double currentYaw = Transformations.quaternionToEulerAngle(currentOrientation)
                .angleZ();
        return Pose.builder()
